...
it appears that for some reason, removing "node\<^sub>i.simps" does not work anymore
*)
...
lemma split_req2:
"\<lbrakk>split xs p = (ls, rs); sorted_less (separators xs)\<rbrakk> \<Longrightarrow>
...
lemma split_req3: "\<lbrakk>split xs p = (ls, rs); sorted_less (separators xs)\<rbrakk> \<Longrightarrow>
case rs of [] \<Rightarrow> True | (sub,sep)#rrs \<Rightarrow> p \<le> sep"
  apply(cases rs)
  using split_req(3)
  by auto

lemma split_req3_alt: 
  assumes "split xs p = (ls,rs)"
...
proof (cases rs)
  ...
  moreover from assms have "sorted_less (separators rs)"
    by (metis map_append sorted_wrt_append split_axioms split_def)
  ultimately have "\<forall>rrsep \<in> set (separators rrs). rsep < rrsep"
    by auto
  moreover have "p \<le> rsep"
    using a_pair assms(1) assms(2) local.Cons split_req3
    by fastforce
...
lemmas split_req_alt = split_req(1) split_req2 split_req3_alt

lemma split_set_ls: "split ts x = (ls,[]) \<Longrightarrow> set ls = set ts"
  using split_conc by fastforce
...
lemma "isin t x \<Longrightarrow> x \<in> set_btree t"
  apply(induction t)
  using split_axioms apply(auto split!: list.splits if_splits dest!: split_set(1))
   apply force+
  done
...
proof(induction n x rule: isin.induct)
  case (2 ts t x)
  then obtain ls rs where list_split[simp]: "split ts x = (ls,rs)" by auto
  ...
  proof
    ...
    next
      ...
      then have "x \<in> set_btree sub"
        using 2 by auto
      then show "x \<in> set_btree (Node ts t)"
        by (metis list_split rs_split child_subset fst_eqD split_set(1) subsetD)
    qed
  qed (auto simp add: "2.IH")
qed simp


(* from the split axioms, we may follow the isin requirements *)
lemma split_separator_match:
...
proof -
  have "x \<in> set (separators (ls@rs))"
    using assms split_req_alt(1)
    by blast
  also have "x \<notin> set (separators ls)"
    using assms(1,3) split_req_alt(2)
    by blast
  ...
  then have "(\<forall>y \<in> set (separators list). x < y)"
    using split_req_alt(3)[of xs x ls rs] assms Cons
    by auto
  then have "x = psep"
...
proof -
  have "\<forall> sep \<in> set (separators ls). sep < x"
    using assms(2,4) split_req_alt(2) sorted_wrt_list_sorted
    by fastforce
  then have "\<forall> (sub, sep) \<in> set ls. x \<notin> set_btree sub"
    by (metis (no_types, lifting) Un_iff assms(3,4) case_prodI2 not_less_iff_gr_or_eq set_append some_child_sub(2) split_req_alt(1) sub_sep_cons.simps)
  moreover have "\<exists>sub \<in> set (subtrees (ls@rs)). x \<in> set_btree sub"
    using assms(1,4) split_req_alt(1)
    by blast
  ...
  then have "x \<le> psep"
    using  split_req_alt(3)[of xs x ls rs] assms Cons sorted_wrt_list_sorted
    by fastforce
  moreover have "\<forall>t \<in> set (subtrees list). \<forall>x \<in> set_btree t. psep < x"
    using sorted_wrt_sorted_left rs_split assms(2,4) split_req_alt(1) sorted_wrt_append sorted_wrt_list_sorted
    by blast
...
proof (cases rs)
  ...
  then obtain sub sep where r_pair: "r = (sub,sep)"
    by (cases r)
  then have "x \<le> sep" 
    using assms split_req_alt Cons r_pair
    by fastforce
  then have "False"
    by (metis r_pair assms(1,2,5) leD Cons some_child_sub(2) split_set(1))
  then show ?thesis
    by simp
qed simp
...
proof (induction t x rule: isin.induct)
  ...
  proof (elim disjE)
    ...
    then have "snd (hd rs) = x" "rs \<noteq> []"
      using list_split split_separator_match assms 2 sorted_wrt_list_sorted
  ...
  next
    ...
    then have "x \<in> set_btree (fst (hd rs))" "rs \<noteq> []"
      using list_split split_subtree_match "2.prems"(1) assms
      by auto
    moreover have "fst (hd rs) \<in> set (subtrees ts)"
      using calculation(2) list_split split_req_alt(1)
      by fastforce
  ...
  next
    ...
    then have "rs = []" 
      using split_last_empty "2.prems"(1) list_split
...
lemma node\<^sub>i_set: "set_up\<^sub>i (node\<^sub>i k ts t) = set_btree (Node ts t)"
proof (cases "length ts \<le> 2*k")
  ...
  then obtain ls sub sep rs where split_half_ts: 
    ...
    by auto
  then have "set_btree (Node ts t) = set_up\<^sub>i (Up\<^sub>i (Node ls sub) sep (Node rs t))"
    using up\<^sub>i_set append_take_drop_id
    by metis
  also have "\<dots> = set_up\<^sub>i (node\<^sub>i k ts t)"
    using False split_half_ts
    by (simp add: node\<^sub>i.simps)
  finally show ?thesis
    by simp
qed (simp add: node\<^sub>i.simps)


lemma ins_set: "set_up\<^sub>i (ins k x t) = set_btree t \<union> {x}"
proof(induction k x t rule: ins.induct)
  case (2 k x ts t)
  then obtain ls rs where split_res: "split ts x = (ls, rs)"
    by (cases "split ts x")
  then have split_app: "ls@rs = ts"
    using split_req_alt(1)
    by auto

  show ?case
  proof (cases rs)
    case Nil
    then have ins_set: "set_up\<^sub>i (ins k x t) = set_btree t \<union> {x}"
    ...
    next
      case (Up\<^sub>i l a r)
      then have "set_up\<^sub>i (node\<^sub>i k (ls@[(l,a)]) r) = set_btree (Node (ls@[(l,a)]) r)"
        using node\<^sub>i_set
        by blast
      also have "\<dots> = set_btree (Node ts t) \<union> {x}"
        using 2 split_app Nil ins_set Up\<^sub>i
        by auto
      finally show ?thesis
        by (simp add: Up\<^sub>i Nil split_res)
    qed
  next
    case (Cons a list)
    ...
    proof (cases "sep = x")
      case False
      then have ins_set: "set_up\<^sub>i (ins k x sub) = set_btree sub \<union> {x}"
      ...
      proof (cases "ins k x sub")
        ...
        also have "\<dots> = set_btree (Node ts t) \<union> {x}"
          ...
          using 2 Cons a_split split_res T\<^sub>i False
          by simp
      next
        case (Up\<^sub>i l a r)
        then have "set_up\<^sub>i (node\<^sub>i k (ls @ (l,a)#(r,sep)#list) t) = set_btree (Node (ls @ (l,a)#(r,sep)#list) t)"
          using node\<^sub>i_set
          by blast
        also have "\<dots> = set_btree (Node (ls@(sub,sep)#list) t) \<union> {x}"
          using ins_set Up\<^sub>i set_btree_split
          by auto
        also have "\<dots> = set_btree (Node ts t) \<union> {x}"
          using split_app Cons a_split
          by simp
        finally show ?thesis 
          using Up\<^sub>i a_split Cons split_app split_res False
          by simp
      qed
    next
      case True
...
lemma up\<^sub>i_sorted:
  assumes "sorted_btree (Node (ls@(sub,(sep::('a::linorder)))#rs) t)"
  shows "sorted_up\<^sub>i (Up\<^sub>i (Node ls sub) sep (Node rs t))"
  unfolding sorted_up\<^sub>i.simps
proof (safe)
...
lemma node\<^sub>i_sorted:
  assumes "sorted_btree (Node ts t)"
  shows "sorted_up\<^sub>i (node\<^sub>i k ts t)"
  using assms
proof(cases "length ts \<le> 2*k")
  case False
  then obtain ls sub sep rs where split_half_ts: 
    ...
    using split_half_not_empty[of ts]
    by auto
  then have "sorted_up\<^sub>i (Up\<^sub>i (Node ls sub) sep (Node rs t))"
    using up\<^sub>i_sorted assms
...
lemma sorted_up\<^sub>i_append:
  assumes "sorted_up\<^sub>i (Up\<^sub>i l a r)"
    and "\<forall>x \<in> set (separators ls). \<forall>y \<in> set_up\<^sub>i (Up\<^sub>i l (a::(_::linorder)) r). x < y"
...
next
  ...
  next
    assume "z = a"
    then show "z < y"
      using \<open>y \<in> set_btree r\<close> assms(1) sorted_up\<^sub>i.simps(2) by blast 
  qed
next
  show
...
(* sorted_less of ins *)
lemma ins_sorted: "sorted_btree t \<Longrightarrow> sorted_up\<^sub>i (ins k (x::('a::linorder)) t)"
proof (induction k x t rule: ins.induct)
  case (2 k x ts t)
  ...
  proof (cases rs)
    ...
    then have ls_sorted: "sorted_btree (Node ls t)" 
      using list_split 2 split_req_alt(1)
      by fastforce
    moreover have ins_sorted: "sorted_up\<^sub>i (ins k x t)"
      using 2 Nil list_split
      by simp
    moreover have ins_set: "\<forall>y \<in> set_up\<^sub>i (ins k x t). \<forall> sep \<in> set (separators ls). sep < y"
    proof -
      have "set_up\<^sub>i (ins k x t) = set_btree t \<union> {x}"
        by (simp add: ins_set)
      then show ?thesis
        using list_split Nil ls_sorted sorted_wrt_list_sorted split_req_alt
        by fastforce
    qed
    show ?thesis
    proof (cases "ins k x t")
      ...
      moreover have "\<forall>y \<in> set_btree a. \<forall> sep \<in> set (separators ls). sep < y"
        using ins_set T\<^sub>i by auto
      ultimately show ?thesis
        using ls_sorted
        by (simp add: T\<^sub>i list_split Nil)
    next
      case (Up\<^sub>i l a r)
      then have "\<forall>x \<in> set (separators ls). \<forall>y \<in> set_up\<^sub>i (Up\<^sub>i l a r). x < y"
        using ins_set Up\<^sub>i by auto
      then have "sorted_btree (Node (ls@[(l,a)]) r)"
        using sorted_up\<^sub>i_append
        by (metis Up\<^sub>i ins_sorted ls_sorted)
      then show ?thesis
        using 2 Up\<^sub>i list_split Nil  node\<^sub>i_sorted[of "ls@[(l,a)]" r]
        by simp
    qed
  next
    case (Cons a list)
    ...
    have sub_lists_sorted:
      ...
      apply (metis (mono_tags, lifting) "2.prems" a_split list_split Cons sorted_btree.simps(2) sorted_wrt_Cons sorted_wrt_split split_req_alt(1) sorted_wrt1)
      apply (metis (mono_tags, lifting) "2.prems" list_split a_split Cons sorted_btree.simps(2) sorted_wrt_Cons sorted_wrt_split split_req_alt(1))
      done
    have ts_split: "ts = ls@(sub,sep)#list"
      using split_req_alt(1) list_split Cons a_split
      by auto
    then have sub_list:
    ...
    next
      case False
      have sub_sorted: "sorted_up\<^sub>i (ins k x sub)"
        using 2 a_split list_split Cons False split_set(1)
        by fastforce
      have sub_set: "set_up\<^sub>i (ins k x sub) = set_btree sub \<union> {x}"
        by (simp add: ins_set)
      ...
      proof (cases "ins k x sub")
        ...
        moreover have "\<forall>x \<in> set (separators ls). \<forall>y \<in> set_btree a. x < y"
          using sub_set
          by (metis (mono_tags, lifting) "2.prems" T\<^sub>i Un_iff ball_empty insertE list_split set_up\<^sub>i.simps(1) sorted_btree.simps(2) sorted_btree_split_ls sorted_wrt_list_sorted split.split_req_alt(2) split_axioms ts_split)
        moreover have "\<forall>x \<in> set (separators list). \<forall>y \<in> set_btree a. x > y"
          using sorted_wrt_sorted_left2
          by (metis (no_types, lifting) "2.prems" T\<^sub>i Un_iff a_split ball_empty case_prodD dual_order.strict_trans insertE less_irrefl list.simps(5) list_split local.Cons set_up\<^sub>i.simps(1) sorted_btree.simps(2) sorted_wrt_list_sorted split.split_req_alt(3) split_axioms split_set(1) sub_lists_sorted(2) sub_sep_cons.simps sub_set)
        moreover have "sub_sep_cons (a,sep)"
          unfolding sub_sep_cons.simps
        proof
          fix y assume "y \<in> set_btree a"
          then have "y \<in> set_btree sub \<or> y = x"
            using T\<^sub>i sub_set by auto
          then show "y < sep"
          proof
            have "sub_sep_cons (sub,sep)"
              using a_split list_split local.Cons split_set(1) sub_list(1) ts_split
              by blast
            then show "y \<in> set_btree sub \<Longrightarrow> y < sep"
              by auto
          next
            have "x < sep" (* TODO make lemma *)
              using split_req_alt ts_split list_split False
        ...
        then show ?thesis
          using 2 a_split list_split Cons False T\<^sub>i
          by simp
      next
        case (Up\<^sub>i l a r)
        ...
        proof (safe)
          ...
          have "set_btree r \<subseteq> set_btree sub \<union> {x}"
            using Up\<^sub>i sub_set by auto
          moreover have "x < sep"
            using False "2.prems" a_split list_split Cons sorted_wrt_list_sorted split_req_alt(3)
            by fastforce
          ultimately have "sub_sep_cons (r, sep)"
            using sub_list
            by fastforce
          moreover have "sub_sep_cons (l,a)"
            using sub_sorted Up\<^sub>i by simp
        ...
        next
          ...
          then show "sorted_btree y"
            using sub_list sub_sorted Up\<^sub>i
            by auto
        next
          show "sorted_btree t"
            using 2 by simp
        next
          ...
          proof
            have "\<forall>y \<in> set_btree sub. y < ty" "x < ty"
              using "2.prems" a_split assms(1) list_split Cons split_set(1) sorted_wrt_list_sorted split_req_alt(3)
              by fastforce+
            moreover have "a \<in> set_btree sub \<union> {x}" using sub_set Up\<^sub>i
              by auto
          ...
          next
            ...
            then show "z < ty"
              by (metis "2.prems" a_split assms(1) list_split Cons separators_split sorted_btree.simps(2) split_req_alt(1))
          qed
        next
          have "sub_sep_sm (l,a) (r,sep)"
          proof -
            have "\<forall>x \<in> set_btree r. a < x"
              using Up\<^sub>i sub_sorted
              by auto
            moreover have "a < sep"
              by (metis (no_types, lifting) False "2.prems" Un_insert_right Up\<^sub>i a_split case_prod_unfold insert_iff less_le list.simps(5) list_split Cons set_up\<^sub>i.simps(2) snd_conv sorted_btree.simps(2) sorted_wrt_list_sorted split_req_alt(3) split_set(1) sub_sep_cons.simps sub_set sup_bot.right_neutral)
          ...
          proof -
            ...
            proof
              ...
              moreover have "ysep < x"
                using "2.prems" y_split y_in_ls list_split sorted_wrt_list_sorted split_req_alt(2)
                by fastforce
          ...
          ultimately show "sorted_wrt sub_sep_sm (ls @ (l, a) # (r, sep) # list)"
            ...
            by fastforce
        qed
        then show ?thesis using 2 a_split list_split Cons False Up\<^sub>i
          by (simp del: node\<^sub>i.simps add: node\<^sub>i_sorted)
          (* FIXME this used to work, and works now, that node\<^sub>i.simps were deleted from the simpset
...
lemma tree\<^sub>i_sorted: "sorted_up\<^sub>i u \<Longrightarrow> sorted_btree (tree\<^sub>i u)"
  apply(cases u)
  apply(auto)
  done

lemma tree\<^sub>i_set: "set_up\<^sub>i u = set_btree (tree\<^sub>i u)"
  apply(cases u)
  apply(auto)
  done
...
next
  ...
  proof (cases "length mts \<ge> k \<and> length tts \<ge> k")
    ...
    proof (cases rs)
      case Nil
      then have
        "set_up\<^sub>i (node\<^sub>i k (mts@(mt,sep)#tts) tt) = set_btree (Node (mts@(mt,sep)#tts) tt)"
        using node\<^sub>i_set
        by blast
      then show ?thesis 
        by (cases "node\<^sub>i k (mts@(mt,sep)#tts) tt")
    ...
    next
      ...
      then have
        "set_up\<^sub>i (node\<^sub>i k (mts@(mt,sep)#rts) rt) = set_btree (Node (mts@(mt,sep)#rts) rt)"
        using node\<^sub>i_set (*former: by (simp del: node\<^sub>i.simps) *) by blast
      then show ?thesis 
        by (cases "node\<^sub>i k (mts@(mt,sep)#rts) rt")
...
proof -
  ...
  proof
    ...
    then have sep_sm: "sep < x"
      using a_split a_set assms(1) assms(3) sorted_wrt_list_sorted split_req_alt(2)
      by fastforce
    then show "y < x"
      using a_set a_split assms(2) assms(3) sep_sm sorted_wrt_sorted_right split_req_alt(1)
      by fastforce
  qed
next
  ...
  proof (cases rs)
    ...
    then obtain sub sep where b_split: "b = (sub,sep)"
      by (cases b)
    then have "sep \<ge> x"
      using assms(1) assms(3) Cons sorted_wrt_list_sorted split_req_alt(3)
      by fastforce
    ...
    proof 
      ...
      proof
        ...
        then show "sep < y"
          by (metis b_split a_set a_split assms(1) assms(3) Cons some_child_sub(1) sorted_wrt_append sorted_wrt_sorted_left split_req_alt(1))
      next
        assume "y = asep"
        then show "sep < y"
          by (metis b_split a_set a_split assms(1) assms(3) Cons sorted_r_forall sorted_wrt_append split_req_alt(1) sub_sep_sm.simps)
      qed
    qed
    moreover have "\<forall>y \<in> set_btree t. y > sep"
      using b_split assms(3) assms(4) Cons split_set(1)
      by fastforce
    ultimately show ?thesis
...
proof(induction k x t rule: del.induct)
  ...
  then have "\<forall>sep \<in> set (separators ls). sep < x"
    by (meson "2.prems"(4) list_split sorted_btree.simps(2) sorted_wrt_list_sorted split_req_alt(2))
  then show ?case
  proof(cases list)
    case Nil
    then obtain lls sub sep where ls_last: "ls = lls@[(sub,sep)]"
      using split_req_alt(1) 2
    ...
    also have "\<dots> = set_btree (rebalance_last_tree k ls (del k x t))"
      ...
      by (metis append_Nil2 bal.simps(2) nonempty_lasttreebal.simps(2) order_bal_nonempty_lasttreebal order_impl_root_order root_order.simps(2) split_req_alt(1))
    finally show ?thesis unfolding del.simps using Nil 2 list_split
      by (metis (no_types, lifting) append_Nil2 case_prod_conv list.simps(4) split_req_alt(1))
  next
    case (Cons a rs)
    then have rs_height: "case rs of [] \<Rightarrow> True | (rsub,rsep)#_ \<Rightarrow> height rsub = height t" (* notice the difference if rsub and t are switched *)
      using "2.prems"(3) bal_sub_height list_split split_req_alt(1) by blast
    ...
    proof cases
      ...
      have sub_height: "height (del k x sub) = height t"
        by (metis "2.prems"(1) "2.prems"(2) "2.prems"(3) a_split bal.simps(2) list_split Cons order_impl_root_order root_order.simps(2) some_child_sub(1) del_height split_set(1))
      from sep_n_x have sub_set: "set_btree (del k x sub) = set_btree sub - {x}"
        by (metis "2.IH"(2) "2.prems" a_split bal.simps(2) list_split Cons order_impl_root_order root_order.simps(2) some_child_sub(1) sorted_btree.simps(2) split_set(1))
      ...
      also have "\<dots> = set_btree (Node (ls@(sub,sep)#rs) t) - {x}"
        ...
        using a_split sep_n_x list_split Cons split_req_alt(1)
        by force
    next
      case sep_x_Leaf
      ...
      also have "\<dots> = set_btree (Node (ls@(Leaf,x)#rs) t) - {x}"
        ...
        using a_split sep_x_Leaf list_split Cons split_req_alt(1)
        by force
    next
      case sep_x_Node
      then obtain sts st where sub_node: "sub = Node sts st" by auto
      from sep_x_Node 2 have "x \<notin> set_btree sub"
        by (metis a_split list_split Cons not_less_iff_gr_or_eq sorted_btree.simps(2) split_set(1) sub_sep_cons.simps)
      ...
      then have set_sub: "set_btree sub = set_btree sub_s \<union> {max_s}"
        using split_max_set
        by (metis "2.prems"(1) "2.prems"(2) "2.prems"(3) a_split bal.simps(2) btree.distinct(1) list_split Cons order_bal_nonempty_lasttreebal order_impl_root_order root_order.simps(2) some_child_sub(1) split_set(1) sub_node)
      ...
      also have "\<dots> = set_btree (Node ts t) - {x}"
        using sep_x_Node Cons a_split list_split split_req_alt(1) by auto
      finally show ?thesis unfolding del.simps
...
next
  ...
  then show ?thesis
    ...
    proof (cases rs)
      ...
      qed auto
      then have
        sorted_node\<^sub>i: "sorted_up\<^sub>i (node\<^sub>i k (mts@(mt,sep)#tts) tt)"
        using node\<^sub>i_sorted by blast

      have "set_up\<^sub>i (node\<^sub>i k (mts@(mt,sep)#tts) tt) = set_btree (Node (mts@(mt,sep)#tts) tt)"
        using node\<^sub>i_set by blast
      also have "\<dots> = set_btree sub \<union> set_btree t \<union> {sep}"
        using  sub_node t_node by auto
      finally have set_node\<^sub>i: "set_up\<^sub>i (node\<^sub>i k (mts@(mt,sep)#tts) tt) = set_btree sub \<union> set_btree t \<union> {sep}"
        by simp
      then show ?thesis
      proof (cases "node\<^sub>i k (mts@(mt,sep)#tts) tt")
        ...
        have "sorted_btree (Node ls u)"
          unfolding sorted_btree.simps
          using assms T\<^sub>i sorted_node\<^sub>i sorted_wrt_append sub_sorted_lr
        proof (safe)
          ...
          then have "x \<in> set_btree t \<or> x \<in> set_btree sub \<or> x = sep"
            using set_node\<^sub>i T\<^sub>i by auto
          then show "lsep < x"
            apply(elim disjE)
        ...
        qed auto
        then show ?thesis
          using T\<^sub>i assms(3) Nil sub_node t_node by auto
      next
        case (Up\<^sub>i l a r)
        then have set_lar:
          ...
          using set_node\<^sub>i by auto
        have "sorted_btree (Node (ls@[(l,a)]) r)"
          unfolding sorted_btree.simps
          using Up\<^sub>i sorted_node\<^sub>i assms 
        proof(safe)
          show "sorted_wrt sub_sep_sm (ls @ [(l, a)])"
            unfolding sorted_wrt_split
          proof (safe)
        ...
        next
          ...
          then show "sub_sep_cons (lsub, lsep)"
            ...
            using Up\<^sub>i sorted_node\<^sub>i by auto
        next
          ...
          then have "\<forall>x \<in> set_btree r. sep < x"
            ...
            using sorted_node\<^sub>i Up\<^sub>i
            apply simp
            done
        ...
        qed auto
        then show ?thesis
          using False Up\<^sub>i Nil sub_node t_node by auto
      qed
    next
      case (Cons r rs)
      then obtain rsub rsep where r_split[simp]:"r = (rsub,rsep)" by (cases r)
      ...
      finally have set_node\<^sub>i: "set_up\<^sub>i (node\<^sub>i k (mts@(mt,sep)#rts) rt) = set_btree sub \<union> set_btree rsub \<union> {sep}"
        by simp
      ...
      qed auto
      then have
        sorted_node\<^sub>i: "sorted_up\<^sub>i (node\<^sub>i k (mts@(mt,sep)#rts) rt)"
        using node\<^sub>i_sorted by blast
      then show ?thesis
      proof(cases "node\<^sub>i k (mts@(mt,sep)#rts) rt")
        ...
        proof (safe)
          ...
          proof (safe)
            ...
            then show "sub_sep_sm (a, b) (u, rsep)"
              ...
              apply (metis (no_types, lifting) T\<^sub>i UnE \<open>(a, b) \<in> set ls\<close> set_node\<^sub>i assms(3) less_trans Cons r_split set_up\<^sub>i.simps(1) singletonD sorted_btree_sorted sorted_inorder_subsepsm sorted_wrt_split2 sub_sep_sm.simps)
              done
          next
            fix a b assume "(a,b) \<in> set rs"
          ...
          then show "sub_sep_cons (a,b)"
            ...
            proof
              ...
              then have "x \<in> set_btree rsub \<or> x = sep \<or> x \<in> set_btree sub"
                using \<open>(a,b) = (u,rsep)\<close> T\<^sub>i set_node\<^sub>i by auto
              then show "x < b"           
        ...
        next
          ...
          show "\<And>x. x \<in> set (subtrees (ls @ (u, rsep) # rs)) \<Longrightarrow> sorted_btree x"
            using assms Cons sorted_node\<^sub>i T\<^sub>i by auto (* this is slow *)
          show "sorted_btree t" using assms by auto
        qed
        then show ?thesis
          using t_node T\<^sub>i Cons sub_node False
          by (auto simp del: sorted_btree.simps node\<^sub>i.simps)
      next
        case (Up\<^sub>i l a r)
        then have set_node\<^sub>i: 
          "set_up\<^sub>i (Up\<^sub>i l a r) = set_btree sub \<union> set_btree rsub \<union> {sep}"
          using set_node\<^sub>i by auto
          ...
          proof (safe)
            ...
            proof -
              ...
              then show "lsep < a \<and> Ball (set_btree l) ((<) lsep)"
                by (metis Un_iff set_node\<^sub>i set_up\<^sub>i.simps(2) singletonI)
            qed
          next
          ...
          next
            ...
            next
              show "\<And>x. x \<in> set_btree r \<Longrightarrow> a < x"
                using sorted_node\<^sub>i Up\<^sub>i by auto
            qed
          next
            show "sorted_wrt sub_sep_sm ls" "sorted_wrt sub_sep_sm rs"
              using sub_sorted_lr Cons by auto
        ...
        next
          ...
          then show "sub_sep_cons (ssub,ssep)"
            using sub_sorted_lr \<open>sub_sep_cons (r,rsep)\<close> sorted_node\<^sub>i Up\<^sub>i Cons
            by auto
        next
          ...
          then show "\<And>x. x \<in> set_btree t \<Longrightarrow> ssep < x"
            ...
            then have "a < x"
              by (metis Un_iff Un_insert_right \<open>x \<in> set_btree t\<close> assms(3) insertI1 set_node\<^sub>i separators_split set_up\<^sub>i.simps(2) sorted_btree.simps(2))
        ...
        next
          ...
          then show "sorted_btree x"
            apply(elim disjE)
            using assms(3) Cons sorted_node\<^sub>i Up\<^sub>i by auto
        next
          show "sorted_btree t" using assms by simp
        qed
        then show ?thesis
          using Cons False Up\<^sub>i sub_node t_node
          by auto
      qed
    qed
  qed simp
...
proof (induction k x t rule: del.induct)
  case (2 k x ts t)
  then obtain ls list where list_split: "split ts x = (ls,list)"
    by (cases "split ts x")
  then show ?case
  proof(cases list)
    case Nil
    ...
    then show ?thesis
      using list_split Nil split_req_alt(1) by force
  next
    case (Cons a rs)
    then obtain sub sep where a_split: "a = (sub,sep)" by (cases a)
    then have node_sorted_split: 
      ...
      using "2.prems" list_split Cons split_req_alt(1) by blast+
    ...
    proof cases
      ...
      then have "set_btree (del k x sub) = set_btree sub - {x}"
        using del_set
        by (metis "2.prems"(1) "2.prems"(2) "2.prems"(3) "2.prems"(4) a_split bal.simps(2) list_split Cons order_impl_root_order root_order.simps(2) some_child_sub(1) sorted_btree.simps(2) split_set(1))
      moreover have "sorted_btree (del k x sub)"
        by (metis "2"(2) "2.prems"(1) "2.prems"(2) "2.prems"(3) "2.prems"(4) sep_n_x  a_split bal.simps(2) list_split Cons order_impl_root_order root_order.simps(2) some_child_sub(1) sorted_btree.simps(2) split_set(1))
    ...
    next
      ...
      proof (safe)
        show "sorted_wrt sub_sep_sm (ls @ (s_sub, s_max) # rs)"
          ...
          by (metis "2.prems"(4) Un_insert_right a_split insertI1 list_split Cons sorted_btree.simps(2) split_set(1) sup.cobounded1)
      next
      ...
      next
        have "s_max < sep"
          using a_split list_split Cons node_sorted_split(1) split_req_alt(1) split_results(2) by fastforce
